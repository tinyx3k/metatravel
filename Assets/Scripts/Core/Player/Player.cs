...
namespace Core.Player
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        [SerializeField] private BoostCurve turning;
        ...
        {
            movingAction.action.Dispose();
            lookingAction.action.Dispose();
            switchCamera.action.Dispose();
        }
        ...
        private void Move(Vector2 direction)
        {
            ...
            OnMoving?.Invoke(speed, direction);
        }
        ...
        private void TurnAround(float posX)
        {
            ...
            if (Math.Abs(_currentYRotation - x) < 0.0001f)
            {
                turning.SetBoostTime(Time.time);
            }
            transform.Rotate(0f, x, 0f);
            _currentYRotation = x;
            var turnOffset = turning.GetBoostTime(Time.time);
            OnTurning?.Invoke(turnOffset,(int)posX);
        }
        ...
        public event Action<float,Vector2> OnMoving;
        public event Action<float,int> OnTurning;
